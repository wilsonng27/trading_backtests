# trading_backtests
  ...
  - `strategy_template_long_only_orb/`: a template strategy folder built on top
  of that engine

  Everything else in the local `trading_backtests/` workspace stays private and
  ...
  Example/template strategy package that imports `backtest_template` and shows
  how to:

  - create a strategy folder outside the engine
  - implement `build_signals(df_exec, df_regime)`
  - configure a runnable `run.py`
  - publish example outputs without exposing private strategy folders
  ...
  <img src="equity_curve.png" alt="Equity Curve" width="900">

  Below is the Monte Carlo result generated by the Strategy Template Long Only
  ORB.

  <img src="monte_carlo.png" alt="Monte Carlo" width="900">
